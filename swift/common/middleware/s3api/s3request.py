...
class ChecksummingInput(InputProxy):
    ...
    def __init__(self, wsgi_input, content_length,
                 checksum_hasher, expected_raw_checksum,
                 expected_trailer=None):
        super().__init__(wsgi_input)
        self._expected_length = content_length
        self._hasher = checksum_hasher
        self.expected_raw_checksum = expected_raw_checksum
        self.expected_trailer = expected_trailer
    ...
    def chunk_update(self, chunk, eof, *args, **kwargs):
        ...
        if error:
            self.close()
            # Since we don't return the last chunk, the PUT never completes
            raise S3InputChecksumMismatch(self._hasher.name.upper())
        return chunk
...
class S3Request(swob.Request):
    ...
    def _install_checksumming_input_wrapper(self):
        ...
        if checksum_headers:
            ...
            checksum_hasher = _get_checksum_hasher(header)
            try:
            ...
            expected_trailer = checksum_trailer_headers[0]
            checksum_hasher = _get_checksum_hasher(expected_trailer)
        ...
        if checksum_algo and checksum_algo.lower() != checksum_hasher.name:
