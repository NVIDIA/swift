...
class ObjectContext(ObjectVersioningContext):
    ...
    def handle_versioned_request(self, req, versions_cont, api_version,
        ...
        if version == 'null':
            resp = req.get_response(self.app)
            if resp.is_success:
                if '%00versions%00' in resp.headers.get(
                        'Content-Location', ''):
        ...
        if is_success(self._get_status_int()):
            ...
            try:
                ...
                while True:
                    ...
                    # determine if a new backend request is necessary
                    if len(listing) > 0 and len(versions_listing) > 0:
                        # make sure to ignore subdir items
                        last_item = [item for item in reversed(listing)
                                     if 'name' in item][0]
                        last_version_item = [item for item in reversed(
                                             versions_listing)
                                             if 'name' in item][0]

                        last_cont = bytes_to_wsgi(
                            last_item['name'].encode('utf-8'))
                        name = self._split_versions_container_name(
                            last_version_item['name'])
                        last_version_cont = bytes_to_wsgi(name.encode('utf-8'))
                        if last_version_cont >= last_cont:
                            break
                        else:
                            # need new listing request
                            params['marker'] = bytes_to_wsgi(
                                last_version_item['name'].encode('utf-8'))
                            params['end_marker'] = \
