...
class NamespaceBoundList(object):
    ...
    def from_namespaces(cls, namespaces):
        ...
        for sr in namespaces:
            if sr.lower < upper:
                # TODO: When we cached full shard ranges, find_shard_ranges
                #   bisected using a sort order of (upper, state, lower) so
                #   overlaps were resolved by state, and a cleaved child
                #   sorted ahead of its parent; now we only have lowers so
                #   the parent ends up being returned by the bisect. Is
                #   discarding the overlap robust reasoning? why did we not
                #   originally make the backend filter overlaps out from
                #   what it returns?
                #   Need to think more about overlaps...if we have:
                #     X(a - d) and Y(c - e) do we discard Y because c < d
                #     or do we store Y but with lower tweaked to 'd'???
                #   Or do we actually need to store uppers and state?!
                # discard overlaps
                # Overlaps are expected during sharding, for example when
                # the backend may return a SHARDING parent and its CLEAVING
                # children. However, will be sorted such that the children
                # precede the parent and it is the children that we prefer.
                continue
