...
class Timestamp(BaseTimestamp):
    ...
    def _offset(self, value):
        if not value:
            return
        value = self._validate_offset(value)

        # XXX how do we encapsulates this in _validate_offset and not
        # re-compuete here?  Maybe MAX_OFFSET = 0xffffff for v1 timestamps?
        if self.hex_part < 0x2000000000000000:
            max_offset = 0x1fffffffffffffff
        elif self.hex_part < 0x3000000000000000:
            max_offset = 0xffffff
    ...
    def __invert__(self):
        if not self.hex_part:
            ...
            inv_hex_part = MAX_HEX_PART + 1 - self.hex_part
        return Timestamp(self._internal_format(inv_float, inv_hex_part))
