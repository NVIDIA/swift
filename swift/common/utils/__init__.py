...
class CooperativeCachePopulator(object):
    ...
    def __init__(self, logger, op_type, infocache, memcache,
                 cache_key, cache_ttl, retry_interval, num_tokens=3):
        self._logger = logger
        ...
        self._req_served_from_cache = False
    ...
    def cache_encoder(self, data):
        """
        To encode data to be stored in Memcached, needs to be implemented by
        sub-class.
    ...
    def cache_decoder(self, data):
        """
        To decode data from Memcached, needs to be implemented by sub-class.
    ...
    def _sleep_and_retry_memcache(self):
        ...
        while cur_time < cutoff_time or num_waits < 3:
            ...
            if cache_data:
                # cache hit.
                self._req_served_from_cache = True
    ...
    def fetch_data(self):
        ...
        if not self._memcache:
            ...
            if self.backend_resp:
                self._logger.increment(
                    'token.%s.backend_reqs.no_cache.%d' %
        ...
        if not self.token_acquired and not self._req_served_from_cache \
                and self.backend_resp:
