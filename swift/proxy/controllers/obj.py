...
class DirectNamespaceCachePopulator(object):
    token_acquired = False

    def __init__(self, ctrl, logger, account, container, req, cache_key):
        self.ctrl = ctrl
    ...
    def fetch_data(self):
        ns_bound_list, self.backend_resp = \
            self.ctrl._get_backend_updating_namespaces(
                self.req, self.account, self.container)
        if ns_bound_list:
            # only store the list of namespace lower bounds and names into
            # infocache and memcache.
            self.set_cache_state = set_namespaces_in_cache(
                self.req, self.cache_key, ns_bound_list,
                self.ctrl.app.recheck_updating_shard_ranges)
...
class CooperativeNamespaceCachePopulator(CooperativeCachePopulator):
    ...
    def __init__(self, ctrl, logger, account, container, req, cache_key):
        ...
        retry_interval = ctrl.app.namespace_cache_token_retry_interval
        num_tokens = ctrl.app.namespace_cache_tokens_per_session
        super().__init__(
            logger, 'shard_updating',
            infocache, memcache, cache_key, cache_ttl,
            retry_interval, num_tokens
        )
        self.ctrl = ctrl
        self.account = account
        self.container = container
