...
class ContainerSharder(ContainerSharderConf, ContainerReplicator):
    ...
    def yield_objects(self, broker, src_shard_range, since_row=None,
        ...
        of undeleted objects are yielded before all batches of deleted
        objects.
        ...
        for include_deleted in (False, True):
            ...
            while True:
                ...
                objects = broker.get_objects(
                    ...
                    since_row=since_row)
                self.logger.debug(
                    'got %s %s objects from %s in %ss',
                    len(objects),
                    'deleted' if include_deleted else 'undeleted',
                    broker.db_file,
                    time.time() - start)
                if objects:
                    yield objects, info
    ...
    def yield_objects_to_shard_range(self, broker, src_shard_range,
        ...
        destination shard ranges provided by the ``dest_shard_ranges``
        function. Yields tuples of (object list, destination shard range in
        which those objects belong). Note that the same destination shard range
        ...
        for dest_shard_range in dest_shard_range_iter:
            ...
            if dest_shard_range:
                sub_src_range = src_shard_range.copy(
                    lower=max(dest_shard_range.upper, src_shard_range.lower))
                for objs, info in self.yield_objects(broker, sub_src_range):
                    yield objs, None, info
