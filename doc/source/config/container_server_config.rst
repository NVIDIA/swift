...
force_replication_mode  auto                        Force a specific replication
                                                    method. By default, the
                                                    replicator uses an automatic
                                                    heuristic to choose between
                                                    rsync and usync replication
                                                    methods. Setting this option
                                                    overrides the heuristic. Valid
                                                    values are: ``auto`` (use
                                                    heuristic), ``rsync`` (always
                                                    use rsync_then_merge), or
                                                    ``usync`` (always send diffs
                                                    over HTTP). Invalid values
...
minimum_shard_size                100000              Minimum size of the final
                                                      ...
                                                      in order to avoid a
                                                      further shard range with
                                                      less than minimum_shard_size
