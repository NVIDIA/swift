...
class TestObjectController(BaseUnitTestCase):
    ...
    def test_DELETE_delete_task_timestamp(self):
        create_object_ts = self.ts()
        update_object_ts = self.ts()
    ...
    def test_stale_delete_task_timestamp_404(self):
        task_timestamp_ts = create_object_ts = self.ts()
        user_deleted_at = self.ts()
        orig_delete_at = utils.normalize_delete_at_timestamp(
            self.ts().normal)
