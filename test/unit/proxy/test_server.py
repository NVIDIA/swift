...
class TestReplicatedObjectController(
    ...
    def test_get_backend_updating_shard_concurrent_reqs_cooperatively(self):
        ...
        self.assertEqual(
            {
                ...
                "object.shard_updating.cache.set.200": 3,
        ...
        stats = self.app.statsd.get_labeled_stats_counts()
        self.assertEqual(
            {('swift_token', frozenset({('account', 'a'),
                                        ('container', 'c'),
                                        ('token', 'with_token'),
                                        ('resource', 'shard_updating'),
                                        ('status', 200),
                                        ('set_cache_state', 'set'),
                                        ('event', 'backend_reqs')})): 3,
             ('swift_token', frozenset({('account', 'a'),
                                        ('container', 'c'),
                                        ('event', 'cache_served'),
                                        ('resource', 'shard_updating'),
    ...
    def test_get_backend_updating_shard_concurrent_reqs_with_failures(self):
        ...
        expected = {
            ...
            'object.shard_updating.cache.set.200': 3 - failures,
        ...
        stats = self.app.statsd.get_labeled_stats_counts()
        expected = {
            ('swift_token', frozenset({('event', 'backend_reqs'),
                                       ('container', 'c'),
                                       ('status', 503),
                                       ('account', 'a'),
                                       ('resource', 'shard_updating'),
                                       ('token', 'with_token')})): failures,
            ('swift_token', frozenset({('event', 'backend_reqs'),
                                       ('token', 'with_token'),
                                       ('set_cache_state', 'set'),
                                       ('container', 'c'),
                                       ('account', 'a'),
                                       ('resource', 'shard_updating'),
                                       ('status', 200)})): 3 - failures,
            ('swift_token', frozenset({('lack_retries', False),
                                       ('resource', 'shard_updating'),
                                       ('token', 'no_token'),
                                       ('container', 'c'),
                                       ('event', 'cache_served'),
                                       ('account', 'a')})): num_processes - 3}
        self.assertEqual(expected, stats)
        self.assertEqual([], self.app.logger.log_dict['set_statsd_prefix'])
        self.assertIn(cache_key, self.memcache.store)
        cached_namespaces = NamespaceBoundList.parse(shard_ranges)
