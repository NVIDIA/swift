...
class ObjectVersioningTestCase(ObjectVersioningBaseTestCase):
    ...
    def test_enable_versioning_existing_container(self):
        ...
        calls = self.app.calls_with_headers
        self.assertEqual(4, len(calls))
        method, path, req_headers = calls[3]
        self.assertEqual('POST', method)
        self.assertEqual('/v1/a/c', path)
...
class ObjectVersioningTestAccountOperations(ObjectVersioningBaseTestCase):
    ...
    def test_list_containers_paging(self):
        listing_body = [{
            'bytes': 10,
            'count': 2,
            'name': 'alpha',
        ...
        params = {
            'format': 'json',
            'prefix': self.str_to_wsgi(get_reserved_name('versions')),
            'marker': self.str_to_wsgi(self.build_container_name('charlie')),
            'end_marker': self.str_to_wsgi(self.build_container_name('zulu')),
        }
        path = '/v1/a?%s' % urllib.parse.urlencode(params)

        self.app.register(
            'GET', path, swob.HTTPOk, {},
            json.dumps(versions_listing_body2).encode('utf8'))
    ...
    def test_list_orphan_hidden_containers(self):
        ...
        params = {
            'format': 'json',
            'prefix': self.str_to_wsgi(get_reserved_name('versions')),
            'marker': self.str_to_wsgi(self.build_container_name('kilo')),
            'end_marker': self.str_to_wsgi(self.build_container_name('zulu')),
        }
        path = '/v1/a?%s' % urllib.parse.urlencode(params)
