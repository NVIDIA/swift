...
class TestCooperativeCachePopulator(unittest.TestCase):
    ...
    class DelayedCachePopulator(CooperativeCachePopulator):
        def __init__(self, logger, op_type, infocache, memcache,
                     cache_key, cache_ttl, retry_interval, backend_delay=0,
                     fetch_backend_failure=False):
            super().__init__(
                logger, op_type, infocache, memcache, cache_key,
                cache_ttl, retry_interval
            )
    ...
    def test_populator_constructor(self):
        ...
        obj = self.MockCachePopulator(
            ...
            self.cache_key, self.cache_ttl, self.retry_interval,
            num_tokens=10
        )
        ...
        self.assertFalse(obj._req_served_from_cache)
    ...
    def test_first_request_with_token(self):
        ...
        self.assertFalse(populator._req_served_from_cache)
        ...
        stats = self.logger.statsd_client.get_increment_counts()
    ...
    def test_no_memcached(self):
        ...
        stats = self.logger.statsd_client.get_increment_counts()
        self.assertEqual({'token.test.backend_reqs.no_cache.200': 1},
                         stats)
    ...
    def test_fetch_data_cache_hit_without_token(self):
        ...
        self.assertFalse(populator.is_token_request_done())
        self.assertTrue(populator._req_served_from_cache)
        stats = self.logger.statsd_client.get_increment_counts()
        self.assertEqual({'token.test.cache_served_reqs': 1},
                         stats)
    ...
    def test_concurrent_requests(self):
        ...
        def worker_process():
            ...
            populator = self.DelayedCachePopulator(
                ...
                self.cache_ttl,
                self.retry_interval,
                backend_delay=self.retry_interval,
            )
            data = populator.fetch_data()
