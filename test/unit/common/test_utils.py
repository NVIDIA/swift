...
class TestShardName(unittest.TestCase):
    def test(self):
        ...
        expected = 'a/root-%s-%s-1' % (parent_hash, ts.normal)
        actual = str(created)
        self.assertEqual(expected, actual)
    ...
    def test_realistic_shard_range_names(self):
        ...
        self.assertEqual('.shards_a/c-hash-%s-42' % ts.normal, str(parsed))

        parsed = utils.ShardName.create('.shards_a', 'c', 'c', ts, 42)
        self.assertEqual(
            '.shards_a/c-4a8a08f09d37b73795649038408b5f33-%s-42' % ts.normal,
            str(parsed))
...
class TestShardRange(BaseUnitTestCase, BaseNamespaceShardRange):
    ...
    def test_shard_range_initialisation(self):
        ...
        expect = dict(name='a/c', timestamp=ts_1.normal, lower='',
                      upper='', object_count=0, bytes_used=0,
                      meta_timestamp=ts_1.normal, deleted=0,
                      state=utils.ShardRange.FOUND,
                      state_timestamp=ts_1.normal, epoch=None,
        ...
        good_run = dict(name='a/c', timestamp=ts_1, lower='l',
                        ...
                        state_timestamp=ts_3.normal, epoch=ts_4,
                        reported=0, tombstones=11)
        expect.update({'lower': 'l', 'upper': 'u', 'object_count': 2,
                       'bytes_used': 10, 'meta_timestamp': ts_2.normal,
                       'state': utils.ShardRange.CREATED,
                       'state_timestamp': ts_3.normal, 'epoch': ts_4,
                       'reported': 0, 'tombstones': 11})
        ...
        assert_initialisation_ok(good_no_meta,
                                 dict(expect, meta_timestamp=ts_1.normal))
    ...
    def _check_to_from_dict(self, lower, upper):
        ...
        sr_dict = dict(sr)
        expected = {
            'name': 'a/test', 'timestamp': ts_1.normal, 'lower': lower,
            'upper': upper, 'object_count': 10, 'bytes_used': 100,
            'meta_timestamp': ts_2.normal, 'deleted': 0,
            'state': utils.ShardRange.FOUND, 'state_timestamp': ts_3.normal,
    ...
    def test_copy(self):
        ...
        self.assertEqual(dict(sr, timestamp=new_timestamp.normal,
                              meta_timestamp=new_timestamp.normal,
                              state_timestamp=new_timestamp.normal),
                         dict(new))
    ...
    def test_make_path(self):
        ...
        parent_hash = md5(b'parent', usedforsecurity=False).hexdigest()
        self.assertEqual('a/root-%s-%s-0' % (parent_hash, ts.normal), actual)
        actual = utils.ShardRange.make_path('a', 'root', 'parent', ts, 3)
        self.assertEqual('a/root-%s-%s-3' % (parent_hash, ts.normal), actual)
        actual = utils.ShardRange.make_path('a', 'root', 'parent', ts, '3')
        self.assertEqual('a/root-%s-%s-3' % (parent_hash, ts.normal), actual)
        actual = utils.ShardRange.make_path(
            'a', 'root', 'parent', ts.normal, '3')
        self.assertEqual('a/root-%s-%s-3' % (parent_hash, ts.normal), actual)
