...
class TestS3ApiMultiUpload(S3ApiBase):
    ...
    def test_object_multi_upload(self):
        ...
        self.assertEqual(1, len(listing_last_modified), listing_last_modified)
        self.assertEqual(
            listing_last_modified[0], copy_resp_last_modified,
            '%r != %r' % (listing_last_modified[0], copy_resp_last_modified))
    ...
    def test_object_multi_upload_part_copy_range(self):
        ...
        listing_last_modified = [p.find('LastModified').text
                                 for p in elem.iterfind('Part')]
        # There should be *exactly* one parts in the result
        self.assertEqual(1, len(listing_last_modified))
        self.assertEqual(
            listing_last_modified[0], copy_resp_last_modified,
            '%r != %r' % (listing_last_modified[0], copy_resp_last_modified))
