...
class InputErrorsMixin(object):
    ...
    def test_good_md5_good_sha_wrong_crc_declared(self):
        resp = self.conn.make_request(
            self.bucket_name,
            'good-checksum',
            method='PUT',
...
class TestV4AuthHeaders(InputErrorsMixin, BaseS3TestCaseWithBucket):
    ...
    def assertInvalidTrailerHeader(self, resp):
        self.assertEqual(resp.status_code, 400, resp.content)
        self.assertIn(b'<Code>InvalidRequest</Code>', resp.content)
    ...
    def test_strm_unsgnd_pyld_trl_declared_no_trailer_sent_ok(self):
        chunked_body = b''.join(
            b'%x\r\n%s\r\n' % (len(chunk), chunk)
            for chunk in [TEST_BODY, b''])
        resp = self.conn.make_request(
    ...
    def test_strm_unsgnd_pyld_trl_with_commas_in_trailer_1(self):
        ...
        resp = self.conn.make_request(
            ...
            headers={
                ...
                'x-amz-decoded-content-length': str(len(TEST_BODY)),
                'x-amz-trailer': ', x-amz-checksum-crc32, ,'})
        self.assertInvalidTrailerHeader(resp)
    ...
    def test_strm_unsgnd_pyld_trl_missing_trailer(self):
        chunked_body = b''.join(
            b'%x\r\n%s\r\n' % (len(chunk), chunk)
            for chunk in [TEST_BODY, b''])
        resp = self.conn.make_request(
    ...
    def test_strm_unsgnd_pyld_trl_multiple_trailers(self):
        chunked_body = b''.join(
            b'%x\r\n%s\r\n' % (len(chunk), chunk)
            for chunk in [TEST_BODY, b''])[:-2]
        chunked_body += ''.join([
    ...
    def test_strm_unsgnd_pyld_trl_multiple_trailers_unsupported(self):
        ...
        resp = self.conn.make_request(
            ...
            headers={
                ...
                'x-amz-decoded-content-length': str(len(TEST_BODY)),
                'x-amz-trailer':
                    'x-amz-checksum-crc32, x-amz-checksum-bad'})
        self.assertInvalidTrailerHeader(resp)
